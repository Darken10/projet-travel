...
@if (!$ticket == null)
    ...
    @else
        @if ($ticket->statut->name == 'Valider')
            <a href="#">
                <div class=" mt-4 flex bg-white border-l-4 border-green-600 rounded-lg p-3 shadow-xl mx-4">
                    ...
                    <div class="">
                        <div>
                            <span class=" font-semibold">{{ $ticket->villeDepart() }} - {{ $ticket->villeArrive() }}</span>
                        </div>
                        <div>
        ...
        @else
        <a href="{{ route('ticket.payer_show',$ticket) }}">
            <div class=" mt-4 flex bg-white border-l-4 border-red-600 rounded-lg p-3 shadow-xl mx-4">
                ...
                <div class="">
                    ...
                    <div>
                        <span class=" italic flex justify-center mx-auto text-sm">{{ $ticket->date }} {{ $ticket->heureDepart() }}</span>
                    </div>
                    <div>
                        <span class=" flex justify-center mx-auto text-sm">{{ $ticket->code }}</span>
                    </div>
                    <div>
                        <span class="  flex justify-center mx-auto text-sm text-nowrap text-red-600 font-semibold">{{ $ticket->numero_tk }}  {{ $ticket->statut->name }}</span>
                    </div>
                </div>
